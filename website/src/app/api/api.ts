...
export default class OCApi {
  ...
  protected post<T>(path: string, sessionToken?: string, body?: any): Observable<T> {
    return this.http.post<T | GenericResponse>(`${this.apiHost}/api/${path}`, body, {
      observe: 'response',
      headers: {
  ...
  protected get<T>(path: string, sessionToken?: string): Observable<T> {
    return this.http.get<T | GenericResponse>(`${this.apiHost}/api/${path}`, {
      observe: 'response',
      headers: {
        'session-token': sessionToken ?? ''
      }
  ...
  protected delete<T>(path: string, sessionToken?: string): Observable<T> {
    return this.http.delete<T | GenericResponse>(`${this.apiHost}/api/${path}`, {
      observe: 'response',
      headers: {
        'session-token': sessionToken ?? ''
      }
