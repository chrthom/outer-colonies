...
export default abstract class EquipmentCard extends Card {
  ...
  getValidTargets(player: Player): CardStack[] {
    return player.cardStacks.filter(
      (cs) => cs.type() == CardType.Hull && cs.profileMatches(this.profile()),
    );
  }
  canAttack(): boolean {
  ...
  attack(weapon: CardStack, target: CardStack): AttackResult {
    ...
    if (
      attackingShip.profile().speed + match.battle.range <=
      target.profile().speed
    )
      damage = Math.round(damage / 2);
    let attackResult = this.attackPointDefense(
      match,
      target,
  ...
  private attackPointDefense(
    ...
    if (attackResult.damage == 0) {
      ...
      const damageReduction =
        this.attackProfile.pointDefense *
        -bestPointDefense.profile().pointDefense;
      if (attackResult.damage >= damageReduction)
        bestPointDefense.defenseAvailable = false;
      const adjustedDamageReduction = Math.min(
        attackResult.damage,
        damageReduction,
      );
  ...
  private attackShield(
    ...
    const bestShield = target
      ...
      .sort(
        (a: CardStack, b: CardStack) => a.profile().shield - b.profile().shield,
      )
      .pop();
    if (attackResult.damage == 0) {
      return attackResult;
      ...
      const damageReduction =
        this.attackProfile.shield * -bestShield.profile().shield;
      if (attackResult.damage >= damageReduction)
        bestShield.defenseAvailable = false;
      const adjustedDamageReduction = Math.min(
        attackResult.damage,
        damageReduction,
      );
  ...
  private attackArmour(
    ...
    const bestArmour = target
      ...
      .sort(
        (a: CardStack, b: CardStack) => a.profile().armour - b.profile().armour,
      )
      .pop();
    if (
      attackResult.damage == 0 ||
      this.attackProfile.armour == 0 ||
      !bestArmour
    ) {
      return attackResult;
    } else {
      const damageReduction =
        this.attackProfile.armour * -bestArmour.profile().armour;
      if (attackResult.damage >= damageReduction)
        bestArmour.defenseAvailable = false;
      const adjustedDamageReduction = Math.min(
        attackResult.damage,
        damageReduction,
      );
