...
test('Single action types shall be consumed from the pool before combinations - version: single first', () => {
  const pool = new ActionPool(new CardAction(CardType.Hull), new CardAction(CardType.Hull, CardType.Equipment, CardType.Infrastructure));
  pool.activate(new Card102());
  expect(pool.pool).toContainEqual(new CardAction(CardType.Hull, CardType.Equipment, CardType.Infrastructure));
});

test('Single action types shall be consumed from the pool before combinations - version: combinations first', () => {
  const pool = new ActionPool(new CardAction(CardType.Hull, CardType.Equipment, CardType.Infrastructure), new CardAction(CardType.Hull));
  pool.activate(new Card102());
  expect(pool.pool).toContainEqual(new CardAction(CardType.Hull, CardType.Equipment, CardType.Infrastructure));
});
