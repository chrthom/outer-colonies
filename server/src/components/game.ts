...
function emitState(io: Server, match: Match) {
  match.forAllPlayers((playerNo: number) => {
    getSocket(io, match, playerNo).emit(
      MsgTypeOutbound.State,
      toClientState(match, playerNo),
    );
  });
...
export function gameSocketListeners(io: Server, socket: Socket) {
  ...
  socket.on(
    ...
    (handCardUUID: string, targetUUID: string) => {
      const match = <Match>socket.data.match;
      const player = getPlayer(socket);
      const handCard = getCardStackByUUID(player.hand, handCardUUID);
      const target = getCardStackByUUID(
        match.getInPlayCardStacks(),
        targetUUID,
      );
      if (!handCard) {
        console.log(
          `WARN: ${player.name} tried to play non-existing card ${handCardUUID}`,
        );
      } else if (!target) {
        console.log(
        ...
        console.log(
          `WARN: ${player.name} tried to play non-playable card ${handCard.card.name}`,
        );
  ...
  socket.on(
    MsgTypeInbound.Retract,
    (cardStackUUID: string, cardIndex: number) => {
      const match = <Match>socket.data.match;
      const player = getPlayer(socket);
      const rootCardStack = getCardStackByUUID(
        player.cardStacks,
        cardStackUUID,
      );
      ...
      if (!rootCardStack) {
        console.log(
          `WARN: ${player.name} tried to retract from non-existing card stack ${cardStackUUID}`,
        );
  ...
  socket.on(MsgTypeInbound.Discard, (handCardUUID: string) => {
    ...
    if (!handCard) {
      console.log(
        `WARN: ${player.name} tried to discard non-existing card ${handCardUUID}`,
      );
    } else {
  ...
  socket.on(
    ...
    (srcId: string, srcIndex: number, targetId: string) => {
      const match = <Match>socket.data.match;
      const player = getPlayer(socket);
      ...
      if (!srcShip) {
        console.log(
          `WARN: ${player.name} tried to attack from non-existing ship ${srcId}`,
        );
      } else if (!srcWeapon) {
        console.log(
        ...
        console.log(
          `WARN: ${player.name} tried to attack non-exisiting target ${targetId}`,
        );
