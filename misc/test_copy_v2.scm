; Test using gimp-image-add-layer
...
(define (main)
  (let* ((template (car (gimp-file-load RUN-NONINTERACTIVE "/home/christopher/Downloads/oc_work/template.xcf" "/home/christopher/Downloads/oc_work/template.xcf")))
    ...
    ; Make bg-group the active layer in template
    (gimp-image-set-active-layer template bg-group)

    ; Copy to clipboard
    (gimp-edit-copy bg-group)

    ; Create a new layer in target
    (let* ((width (gimp-drawable-get-width bg-group))
      ...
      (gimp-image-add-layer target new-layer 0)
      ...
      (let ((floating (car (gimp-edit-paste new-layer #f))))
        ...
        (gimp-message "Anchored"))

      ; Save
      (gimp-file-save RUN-NONINTERACTIVE target (car (gimp-file-new "/home/christopher/Downloads/oc_work/output/test_v2.xcf")))
      (gimp-message "Saved!")))
